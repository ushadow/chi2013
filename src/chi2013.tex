...
probabililty of targeting ``E''. This is because the Gaussian models for the ``W'' and ``E'' keys have left
horizontal shifts from their corresponding centers for the two-thumb input posture. 
...
on-device simulation and show that posture and individual adapation can improve the key probability
estimation.
...
a bivariate Gaussian distribution, and the keyboard only started adatping
...
studied vidually adaptive nine-key numeric keypads for individuals.  
...
affect key probability estimation accuracy.
...
The simpliest base model can have a Gaussian distribution with 0 mean $x$ and $y$
...
One step beyond this is having a full covriance matrix $\Sigma$ trained from the
...
\cite{Rudchenko:2011}). A closer look at the data shows that the most number of
errors occurs between ``E'' and ``W''. Because there are more one-finger input users in the data set, the Gaussian models for ``E'' and ``W'' have right horizontal offsets. However, for
the two-thumb input, there is usually a left horizontal offsets. Hence there are
more errors for the two-thumb input when we use the key adaptive model.

This suggests that key adpative model may not always be effectivce unless we
...
Due to the scope of this paper, however, we do not analyze further about the
effectiveness of adapting to bi-letter patterns, but only leave it as a future
work.
...
There can be different levels of complexy for posture adapation. For the most complex one, we can build two models for each key for the 
...
Another point to note is that the error rate is based on perfect knowledge of
the posture which represents an upper bound for posture adaptation for this data
set. In the real world implementation, the online posture classification may
introduce errors. To mitigate the propblem, the posture model can be turned on
...
of data points, we only show the trend when the nubmer used to build the user
model for each key is 70.
However, the figure still shows a general trend, and suggests that a number in 
the order of a hundred is probably a good nubmer. When we do not have enough 
...
can give higher prirority to user adaptation when we do not have enough data for
the highest order model (the posture, user and key adaptive model). This
suggests the following parital hierarchy and back-off paths. The result in the
...
they can help with the classfication. Other input postures such as ten-finger input
