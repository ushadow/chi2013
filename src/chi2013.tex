...
The hierarchical adaptive SBM consists of a number of submodels in different “orders” (Figure. \ref{fig:hierarchy}). Each sub-model is represented by a bivariate Gaussian distribution $N(\underline\mu, \Sigma)$
...
\begin{figure}[tb]
  ...
  \caption{A complete hierarchical spatial backoff model.}
  \label{fig:hierarchy}
\end{figure}
...
possible combinations of the three adaptive factors. The ``posture adaptive’’ model, for example, adapts to input posture only which means there is one Gaussian model $N(\underline\mu_y, \Sigma_y)$ for each posture $y$ for all keys. The ``posture & key adaptive’’ model adapts to both posture and key which means there is one Gaussian model $N(\underline\mu_{y, k}, \Sigma_{y, k})$ for each posture $y$ and key $key$ combination. However, depending on the
